...
const Profile = () => {
  ...
  const nonFriendsUsers = useMemo(
    () =>
      users?.results?.filter(
        (u) =>
          !(friends?.results?.some((f) => f.friendId === u.id) ?? true) &&
          u.id !== userProfile.id
      ),
    [users, friends]
  )
  ...
  const filteredPendingRequests = useMemo(
    () => pendingRequests?.filter((request) => !request.isMirror) ?? [],
    [pendingRequests]
  )
          ...
          filteredPendingRequests.length === 0 ? (
            ...
            filteredPendingRequests.map((friend) => (
              <FriendCard
                ...
                mutatePendingRequests={mutatePendingRequests}
              />
            ))
          )}
        </div>
          ...
          {nonFriendsUsers?.length === 0 ? (
            ...
            nonFriendsUsers?.map((user) => (
              <FriendCardSend
                ...
                mutatePendingRequests={mutatePendingRequests}
                isPendingRequest={pendingRequests?.some(
                  (f) => f.userId === user.id
                )}
              />
            ))
