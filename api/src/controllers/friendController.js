...
const addFriend = async ({ userEmail, friendEmail }) => {
    try {
        ...
        if (!friend1) {
            throw new Error(`No se encontro un usuario con email: ${userEmail}`);
        }
        ...
        if (!friend2) {
            throw new Error(`No se encontro un usuario con email: ${friendEmail}`);
        }
        ...
        // Marcar la solicitud espejo con el campo isMirror en true
        inverseFriendship[0].isMirror = true;
        await inverseFriendship[0].save();
